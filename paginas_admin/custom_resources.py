...
def custom_resources():
    ...
    new_amount = col5.number_input("Cantidad del recurso a agregar",0)
    edit_amount = col6.number_input("Cantidad del recurso a editar",0)
    options = []

    col1, col2, col3 = st.columns(3)
    for k,v in st.session_state.custom_resources.items():
        options.append(f"{k}: {v}")
    element = st.radio("Lista de recursos personalizados",options,horizontal=True)
    if element != None:
    ...
    col1.button("Agregar",on_click=lambda: st_resources_edit(new_resource,new_amount),disabled=not_addition,type="primary",width="stretch",help="Rellene primero el nombre del recurso y la cantidad disponible")
